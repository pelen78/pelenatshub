...
The PLC loads this catalogue through the authenticated projects endpoint. NOW activities are suggested. Each PLC evaluates one project, with its own observations, stages and commitment; a subject/week can have one PLC per project. Older PLCs that already list several projects keep their shared answers unchanged and are labelled as shared. The Plan stage is labelled as looking ahead, so a later project named there is not confused with the project evaluated. Every PLC stores a snapshot; subsequent assignment edits and removals leave prior weekly records intact. Editing a project snapshot in a PLC does not change the public assignment.
