...
export async function authenticate(request, env, verify = jwtVerify) {
  if (!env.ACCESS_TEAM_DOMAIN || !env.ACCESS_AUD || !env.ADMIN_EMAILS || !env.GITHUB_TOKEN) {
    return { status: 503, error: 'Access and the GitHub connection must be configured in Cloudflare.' };
  }
  const domain = env.ACCESS_TEAM_DOMAIN.replace(/^https:\/\//, '').replace(/\/$/, '');
  if (!/^[a-z0-9-]+\.cloudflareaccess\.com$/.test(domain)) return { status: 503, error: 'Invalid access configuration.' };
  const token = request.headers.get('Cf-Access-Jwt-Assertion');
  if (!token) return { status: 401, error: 'Sign in to the dashboard with an authorized email address.' };
  try {
    ...
    if (!payload.exp || typeof payload.email !== 'string' || !env.ADMIN_EMAILS.split(',').map(email => email.trim().toLowerCase()).includes(payload.email.toLowerCase())) {
      return { status: 403, error: 'This email address does not have publishing permission.' };
    }
    return { email: payload.email };
  } catch {
    return { status: 401, error: 'Your session expired. Sign in to the dashboard again.' };
  }
}
