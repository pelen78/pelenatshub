// Adds a fixed "← Class Hub" button to an activity page so students can get back to their subject.
...
export function addBackLink(html, link, group) {
  ...
  const button = `<a data-hub-back href="${hubHref(link, group)}" style="position:fixed;top:12px;left:12px;z-index:2147483000;display:inline-flex;align-items:center;gap:6px;padding:8px 14px;border-radius:999px;background:#12275C;color:#fff;font:600 14px/1.2 system-ui,-apple-system,'Segoe UI',sans-serif;text-decoration:none;box-shadow:0 2px 10px rgba(11,23,64,.3);border:1.5px solid #F3B700">← Class Hub</a><style>@media print{[data-hub-back]{display:none!important}}</style>`;
